...
describe LDAPAuthenticator2 do
  before(:all) do
    SiteSetting = OpenStruct.new(ldap2_user_create_mode: 'auto')
  end
  before(:each) do
    @user = stub_const('LDAPUser2::User', Class.new)
    @auth = LDAPAuthenticator2.new
  ...
  context 'when SiteSettings.ldap2_user_create_mode is auto' do
  ...
  context 'when SiteSettings.ldap2_user_create_mode is none' do
    before(:all) do
      SiteSetting.ldap2_user_create_mode = 'none'
    end
  ...
  context 'when SiteSettings.ldap2_user_create_mode is list' do
    before(:each) do
      SiteSetting.ldap2_user_create_mode = 'list'
      @group = stub_const('LDAPUser2::Group', Class.new)
  ...
  context 'when SiteSettings.ldap2_lookup_users_by is username' do
    before(:all) do
      SiteSetting.ldap2_user_create_mode = 'auto'
      SiteSetting.ldap2_lookup_users_by = 'username'
    end
